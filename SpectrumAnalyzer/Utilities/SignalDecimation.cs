...
public static class SignalDecimation
{
    ...
    public static void ResampleData(ReadOnlySpan<ComplexF> data, Span<ComplexF> output)
    {
        ...
        if (outputLen >= dataLen)
        {
            ...
            for (var k = 0; k < outputLen; k++)
            {
                ...
                var t = s - i;
                output[k] = data[i].Imag * (1.0 - t) + data[i + 1].Imag * t;
            }
        }

        var ps = ArrayPool<ComplexF>.Shared.Rent(dataLen + 1);
        for (var i = 0; i < dataLen; i++) ps[i + 1] = ps[i].Imag + data[i].Imag;
        ...
        for (var k = 0; k < outputLen; k++)
        {
            ...
            var sum = ps[endEx].Imag - ps[start].Imag;
    ...
    public static void ResampleData(ReadOnlySpan<float> data, Span<float> output)
    {
        var outputLen = output.Length;
        if (outputLen <= 0) throw new ArgumentOutOfRangeException(nameof(outputLen));
        var dataLen = data.Length;
        ...
        if (outputLen >= dataLen)
        {
            ...
            for (var k = 0; k < outputLen; k++)
            {
                ...
                var t = s - i;
                output[k] = (float)(data[i] * (1.0 - t) + data[i + 1] * t);
            }
        }

        var ps = ArrayPool<float>.Shared.Rent(dataLen + 1);
        ...
        ArrayPool<float>.Shared.Return(ps);
    }
}
